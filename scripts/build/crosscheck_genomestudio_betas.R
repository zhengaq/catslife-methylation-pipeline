#!/usr/bin/env Rscript
### scripts/build/crosscheck_genomestudio_betas.R: optional check, outside the normal run.
### Compares this pipeline's minfi -> noob -> dasen betas (F_DASENB) with GenomeStudio's
### Methylation_Profile.txt AVG_Beta values on a small random subset of probes and samples.
...
if (!all(c("Sample_Group", "Sample_Name") %in% names(sheet)))
    stop("crosscheck_genomestudio_betas: SAMPLE_SHEET has no Sample_Name column, so it ",
         "can't be mapped to Methylation_Profile.txt's per-sample column prefixes.")
...
if (!length(have_cols))
    stop("crosscheck_genomestudio_betas: none of this cohort's samples have a matching ",
         "<Sample_Name>.AVG_Beta column in Methylation_Profile.txt; wrong file or wave?")
...
if (any(low_r, na.rm = TRUE))
    cat("\nWARNING: pearson_r < 0.9 for:", paste(out$Sample_Group[which(low_r)], collapse = ", "),
        "\nGenomeStudio computes betas differently from minfi/noob/dasen, so some deviation is",
        "expected; a very low r can point to a sample or probe identity mismatch.\n")
